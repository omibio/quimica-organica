    ...
    for kp, vp in pairs(prefixo) do
        for ki, vi in pairs(infixo) do
            ...
            if findp and findi then
                return keyp, str, keyi, vai, cicleno ~= nil, dupla, ram ~= nil, subidentificar(
                    prefixo,
                    ram,
                    tram,
                    tram2
...
function ch(v, ks, x)
    x[v] =
        "CH" ..
        4 - diminuir(x, y, keyi, l, i, keyp) - diminuir2(l, y, keyi, keyp) - ddiminuir(dupla, tabelad, keyi, str, x) -
            rdiminuir(ks, i)
    if x[v] == "CH1" then
        x[v] = "CH"
    elseif x[v] == "CH0" then
